...
void print_files_received(ArrayList *files)
{
    ...
    for (int i = 0; i < files->count; i++)
    {
        ...
        printf(" | ");
        printf("%-*d", max_fsize_size, (size_t)((ls_files *)files->elements)[i].file.fsize);
        printf(" | ");
        printf("%s:%d", inet_ntoa(((peer *)((ls_files *)files->elements)[i].holders->elements)[0].con.sin_addr), ntohs(((peer *)((ls_files *)files->elements)[i].holders->elements)[0].con.sin_port));
        for (int j = 1; j < ((ls_files *)files->elements)[i].peers_size; j++)
        {
            printf(", %s:%d ", inet_ntoa(((peer *)((ls_files *)files->elements)[i].holders->elements)[j].con.sin_addr), ntohs(((peer *)((ls_files *)files->elements)[i].holders->elements)[j].con.sin_port));
        }
        printf("\n");
    }
}
    ...
    for (int i = 0; i < rec_files_len; i++)
    {
        ...
        else
        {
            ...
            if (position != j)
            {
                ...
                a.file.fname = fname;
                a.file.fsize = fsize;
                a.holders = alloc_list(sizeof(peer)),
                a.peers_size = 0,
                append_element(list, (void *)&a);
            }
