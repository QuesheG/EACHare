...
#define msg_size_files_list(x) (MSG_SIZE + (x * 276)) //MSG_SIZE + (x * sizeof(name:int)) (name => 255(linux) int 20 )
...
void share_files_list(peer *server, pthread_mutex_t *clock_lock, SOCKET con, char *dir_path); //send list of files 
void send_file(peer *server, pthread_mutex_t *clock_lock, SOCKET con, char *dir_path, dl_msg_args *arg); //send file
...
char *get_file_in_msg(const char *buf,int *a, int *b); //return the file in base64 format
